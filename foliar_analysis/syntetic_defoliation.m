...
%   image
%
%   max_size_leaf_scale_factor - the maximum scale factor for the leaf
%   image
%
...
% [leaf_out, bite_signature, img_out, leaf_seg, defoliation_level] = syntetic_defoliation(img, 5, 'caterpillar_bite', 15, 30, 30, 50, 1);

function [leaf_out, bite_signature, img_out, leaf_seg, defoliation_level, damaged_areas] = ...
    syntetic_defoliation(img, filter_size, path_bites, min_defoliation,...
...
leaf_seg2_mask = leaf_seg(:,:,2) > 0;
leaf_inner_holes = leaf_mask & ~leaf_seg2_mask;
qtty_leaf_inner_holes = sum(leaf_inner_holes(:));
...
function [mask_leaf_after, mask_leaf_before, defoliation_level] = ...
    ...
    height, width, leaf_mask, qtty_leaf_inner_holes, only_border_damage)
...
%defoliation_level = 100 - ( ( sum(mask_leaf(:)) / sum(leaf_mask(:)) ) .* 100 );
diff = leaf_mask & ~mask_leaf;
defoliation_level = ( ( sum(diff(:)) + qtty_leaf_inner_holes ) / sum(leaf_mask(:)) ) *100;
